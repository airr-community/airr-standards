...
# to be uncommented when we drop python 2 support
...
def merge_two_dicts(x, y): # remove when python 2 support is dropped and use code from above for 3.5+
    z = x.copy()
    z.update(y)
    return z

test_files = merge_two_dicts({"python": {basename(f): f for f in glob('lang/python/tests/data/*')}},
              {"R": {basename(f): f for f in glob('lang/R/tests/data-tests/*')}})
spec_files = merge_two_dicts((merge_two_dicts({"spec": {basename(f): f for f in glob('specs/airr-schema.yaml')}},
              {"python": {basename(f): f for f in glob('lang/python/airr/specs/airr-schema.yaml')}})),
            {"R": {basename(f): f for f in glob('lang/R/inst/extdata/airr-schema.yaml')}})
